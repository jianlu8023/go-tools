...
func TestCompressAndDecompress(t *testing.T) {
	// 测试数据
	testData := []byte("Hello, this is a test for s2 compression!")
...
func TestCompressFileAndDecompressFile(t *testing.T) {
	// 创建临时目录
	tempDir, err := os.MkdirTemp("", "s2-test")
	if err != nil {
	...
	testContent := []byte("This is a test file content for s2 compression and decompression")
	if err := os.WriteFile(srcPath, testContent, 0644); err != nil {
		t.Fatalf("Failed to write test file: %v", err)
	}

	// 压缩文件
	compressedPath := filepath.Join(tempDir, "test.txt.s2")
