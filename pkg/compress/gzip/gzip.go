...
// Decompress 使用gzip算法解压缩字节数据
func Decompress(data []byte) ([]byte, error) {
	return DecompressWithLimit(data, 0)
}

// DecompressWithLimit 使用gzip算法解压缩字节数据，并限制最大解压大小
// 如果maxSize为0，则不限制大小
...
func CompressFile(srcPath, destPath string) (err error) {
	...
	// gzip.Writer.Close 负责写入尾部校验和，必须显式关闭并检查错误；
