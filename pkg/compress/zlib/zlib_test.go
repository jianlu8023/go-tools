...
func TestCompressAndDecompress(t *testing.T) {
	// 测试数据
	testData := []byte("Hello, this is a test for zlib compression!")
...
func TestCompressFileAndDecompressFile(t *testing.T) {
	// 创建临时目录
	tempDir, err := os.MkdirTemp("", "zlib-test")
	if err != nil {
	...
	testContent := []byte("This is a test file content for zlib compression and decompression")
	if err := os.WriteFile(srcPath, testContent, 0644); err != nil {
		t.Fatalf("Failed to write test file: %v", err)
	}

	// 压缩文件
	compressedPath := filepath.Join(tempDir, "test.txt.zlib")
