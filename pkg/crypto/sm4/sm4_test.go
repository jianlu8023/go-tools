...
func tempDir(t *testing.T) string {
	tempDir := filepath.Join(os.TempDir(), "sm4_test")
...
func TestCTREncryptFile(t *testing.T) {
	...
	content := "this is a sm4 crypto test file"

	if err := os.WriteFile(original, []byte(content), 0644); err != nil {
		t.Fatal(err)
	}
	if err := CTREncryptFile(key, original, encryptedctr); err != nil {
		t.Fatal(err)
	}
}
...
func TestCTRDecryptFile(t *testing.T) {
	...
	// 再解密
	if err := CTRDecryptFile(key, encryptedctr, decryptedctr); err != nil {
		t.Fatal(err)
	}
...
func TestCBCEncryptDecryptBytes(t *testing.T) {
	plaintext := []byte("Hello, SM4 CBC bytes encryption!")
	keyBytes := []byte(key)
...
func TestCTREncryptDecryptBytes(t *testing.T) {
	plaintext := []byte("Hello, SM4 CTR bytes encryption!")
	keyBytes := []byte(key)
...
func TestKeyLengthValidation(t *testing.T) {
	...
	longKey := []byte("this_is_a_very_long_key_that_exceeds_16_bytes") // 长密钥
	text := "test text"
