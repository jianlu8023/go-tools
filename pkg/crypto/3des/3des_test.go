...
func tempDir(t *testing.T) string {
	tempDir := filepath.Join(os.TempDir(), "3des_test")
...
func TestCBCEncryptFile(t *testing.T) {
	...
	content := "this is a 3des crypto test file"
...
func TestCBCDecryptFile(t *testing.T) {
	tempDir := tempDir(t)
	original := filepath.Join(tempDir, "original.txt")
	encryptedCbc := filepath.Join(tempDir, "encryptedCbc-cbc.txt")
	decryptedCbc := filepath.Join(tempDir, "decryptedCbc-cbc.txt")
	content := "this is a 3des crypto test file"
	...
	// 先加密
	if err := CBCEncryptFile(key, original, encryptedCbc); err != nil {
		t.Fatal(err)
	}

	// 再解密
	if err := CBCDecryptFile(key, encryptedCbc, decryptedCbc); err != nil {
		t.Fatal(err)
	}
...
func TestCBCEncryptDecryptText(t *testing.T) {
	text := "Hello, 3DES CBC encryption!"
	keyBytes := []byte(key)
...
func TestCTREncryptDecryptText(t *testing.T) {
	text := "Hello, 3DES CTR encryption!"
	keyBytes := []byte(key)
...
func TestCBCEncryptDecryptBytes(t *testing.T) {
	plaintext := []byte("Hello, 3DES CBC bytes encryption!")
	keyBytes := []byte(key)
...
func TestCTREncryptDecryptBytes(t *testing.T) {
	plaintext := []byte("Hello, 3DES CTR bytes encryption!")
	keyBytes := []byte(key)
...
func TestKeyLengthValidation(t *testing.T) {
	...
	longKey := []byte("this_is_a_very_long_key_that_exceeds_24_bytes") // 长密钥
	text := "test text"
