...
func CBCEncrypt(key, plaintext []byte) ([]byte, error) {
	// 验证密钥长度
	if len(key) != 24 {
		return nil, fmt.Errorf("3DES密钥长度必须为24字节，当前长度为%d字节", len(key))
	}

	block, err := des.NewTripleDESCipher(key)
	if err != nil {
		return nil, fmt.Errorf("创建3DES块错误: %v", err)
	}
...
// CBCEncryptText 使用3DES CBC模式加密文本
// key: 24字节的密钥
// text: 明文文本
// 返回加密后的字节切片和错误信息
...
// CBCDecryptText 使用3DES CBC模式解密文本
// key: 24字节的密钥
// ciphertext: 密文字节切片（包含IV）
// 返回解密后的文本和错误信息
...
// CBCEncryptFile 使用3DES CBC模式加密文件
// key: 24字节的密钥
// inputFile: 输入文件路径
// outputFile: 输出文件路径
// error: 错误信息
func CBCEncryptFile(key, inputFile, outputFile string) error {
	...
	if err != nil {
		return fmt.Errorf("创建3DES块错误: %v", err)
	}
...
// CBCDecryptFile 使用3DES CBC模式解密文件
// key: 24字节的密钥
// inputFile: 输入文件路径
// outputFile: 输出文件路径
// error: 错误信息
func CBCDecryptFile(key, inputFile, outputFile string) error {
	...
	// 移除填充
	unPaddingBuf, err := pkcs5UnPadding(plaintext)
	if err != nil {
		return err
	}
