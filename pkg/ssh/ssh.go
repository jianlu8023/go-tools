...
func ExecuteCommand(user, password, host, port, command string) (string, error) {
	...
	defer func(client *ssh.Client) {
		err := client.Close()
		if err != nil {
			fmt.Printf("executeCommand 关闭ssh客户端发生错误: %s\n", err.Error())
		}
	}(client)
	...
	defer func(session *ssh.Session) {
		...
		if err != nil {
			if err == io.EOF {
				fmt.Println("executeCommand 关闭ssh会话成功")
			} else {
				fmt.Printf("executeCommand 关闭ssh会话发生错误: %s\n", err.Error())
			}
		}
	}(session)
...
func ExecuteCommands(user, password, host, port string, commands []string) ([]string, error) {
	...
	defer func(client *ssh.Client) {
		err := client.Close()
		if err != nil {
			fmt.Printf("executeCommands 关闭ssh客户端发生错误: %s\n", err.Error())
		}
	}(client)
	...
	defer func(session *ssh.Session) {
		...
		if err != nil {
			if err == io.EOF {
				fmt.Println("executeCommands 关闭ssh会话成功")
			} else {
				fmt.Printf("executeCommands 关闭ssh会话发生错误: %s\n", err.Error())
			}
		}
	}(session)
